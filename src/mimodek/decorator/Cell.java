...
/**
 * A cell of the DLA. Each cell can have one or more neighbours. Other class should only communicate with the DLA through one and one cell only.
 * Message like attract or repel are passed to neighbours cell and so on, so the whole tree will be queried by calling the method only on a single cell (usually the first one created).
 * Update of two cells from the same DLA will have unexpected result and in any case be take much more time. Tgis also why most methods takes a calling Cell as a parameter so the message will not go back to a cell that has been called already. 
 * This class subclass MimodekObjectGraphicsDecorator and decorates only other MimodekObjectGraphicsDecorator because of this: the draw method must call the other cells draw method and so on to display the whole tree. It does not however do any more drawing than the decorated object.
 * Bottom line is : one DLA = one instance of a Cell in the MimoManager class, no more.
...
public class Cell extends MimodekObjectGraphicsDecorator {
	 ...
	 * @param c a neighbour to remove from the list
	 */
	 ...
	 * @param up The calling cell
	 */
	protected void update(Cell up) {
		float d = 0;
		if (!fixed && up != null){ // calculate the distance between the calling cell and this one
			d = getPos().dist(up.getPos()) - (up.getDiameter() + getDiameter())/2;
			if (d > -3) {
			...
			/*else if(d<0){
				//move to maintain the distance between cells
				float a = (float) Math.atan2(getPosY()-up.getPosY(),getPosX()-up.getPosX());
				PVector nuPos = new PVector(getPosX() + PApplet.cos(a) * d,
						getPosY() + PApplet.sin(a) * d);
				if (FacadeFactory.getFacade().isInTheScreen(nuPos,
						getRadius() / 2))
	...
	/**
	 * Attach a cell to this cell.
	 * You should perform a attachTest() first.
	 * @param c The cell to be attached
	 */
	...
	/**
	 * Attract cells if they are close enough (depending on the threshold) to the coordinates
	 * @param up The calling Cell
	 * @param x Horizontal coordinate
	 * @param y Vertical coordinate
	 * @param threshold The radius of influence
	 */
	public void attract(Cell up, float x, float y, float threshold) {
		...
		if (!fixed && d <= threshold /*&& d >= getRadius()/2*/) {
			...
			PVector nuPos = new PVector(getPosX() + PApplet.cos(a) * d,
					getPosY() + PApplet.sin(a) * d);
			if (FacadeFactory.getFacade().isInTheScreen(nuPos, getDiameter() / 2))
				setPos(nuPos);
			update(null);
		}
		for (int i = 0; i < neighbours.size(); i++) {
	...
	/**
	 * Repel cells if they are close enough (depending on the threshold) to the coordinates
	 * @param up The calling Cell
	 * @param x Horizontal coordinate
	 * @param y Vertical coordinate
	 * @param threshold The radius of influence
	 ...
	 * Find all the leaves.
	 * @param up The calling Cell
	 * @param collector This ArrayList will contain all the leaves found
	 */
