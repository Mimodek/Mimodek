...
public class Physics {
	...
	public void drawSprings(int c, float strokeW){
		...
		for(int i=-1;++i<n;){
			...
			Simulation1.gfx.line(p1.position().x(),p1.position().y(),p2.position().x(),p2.position().y());
		}
	...
	public void addSpring(Particle a, Particle b){
		if(areAlreadyConnected(a, b))
			return;
		physics.makeSpring( a, b, springStrength, springDamping, PApplet.dist(a.position().x(), a.position().y(), b.position().x(), b.position().y()));
	}
