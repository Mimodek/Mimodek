...
public class Plotter {
	...
	public Plotter(int width, int height, float min, float max, float space) {
		...
		this.max = max;
		this.space = space;
		buffer = GUI.gui().app.createGraphics(width,height,PApplet.JAVA2D);
		buffer.beginDraw();
		buffer.fill(GUI.gui().app.color(0,0,0,0));
	...
	public void plot(float value){
		...
		if(lastFrame != 0){
			...
			x = lastPoint.x + (GUI.gui().app.frameCount - lastFrame)*space;
			
			buffer.line(lastPoint.x,lastPoint.y,x,y);
			buffer.endDraw();
		}
		lastPoint = new PVector(x,y);
		lastFrame = GUI.gui().app.frameCount;
	}
}
