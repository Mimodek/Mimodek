...
public class RadialGradient implements SimpleDrawer {
	...
	public PImage draw(Object drawingData) {
		...
		PGraphics buffer = MainHandler.app.createGraphics((int)data.maxRadius, (int)data.maxRadius, PApplet.JAVA2D);
		
		buffer.beginDraw();
		buffer.background(0,0,0,0);
		buffer.noStroke();
		for(int i=(int) data.radius;i>=0;i--){
			buffer.fill(MainHandler.app.lerpColor(data.startColor, data.endColor, (float)i/data.maxRadius));
