...
public class CellA extends Cell {
	...
	*/
	...
	void setAnchor(Cell anchor){
		...
		angularMovement = (float) (Math.random() * (PApplet.HALF_PI / 10f));
	...
	public void draw(PApplet app) {
		...
		int frameCount = Mimodek.FRAME_COUNT;
...
//*/
	...
	gl.glMultiTexCoord2f(GL.GL_TEXTURE1, 0.5f, 0.5f);
		gl.glVertex2f(0f, 0f);
			...
			for (float a = PApplet.TWO_PI / Mimodek.CELLA_VERTEX_NUMBER; a < PApplet.TWO_PI; a += PApplet.TWO_PI
	...
	public static CellA addCellA(PApplet app) {
		...
		while (!addIt && counter < Configurator.getIntegerSetting("CELLA_MAX_TRY")) {
			if (!FacadeFactory.getFacade().isInTheScreen(pos, radius*radiusModifier)) {
				...
				a = (float) Math.random() * Mimodek.TWO_PI;
		...
		if (counter >= Configurator.getIntegerSetting("CELLA_MAX_TRY")) {
			return null;
		}
		// println(added.pos);
		return added;
	...
	public static void setOpenGlState() {
		...
		OpenGL.multiTexture(TextureManager.getTextureIndex(Configurator.getStringSetting("CELLA_MASK")),TextureManager.getTextureIndex(Configurator.getStringSetting("CELLA_TEXTURE")));
	}
