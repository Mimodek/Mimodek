...
public class TextureManager {
	...
	public static void loadAllTextureInFolder(PApplet app, String folderName){
		//String sketchPath = app.sketchPath;
		File folder = new File(app.sketchPath+"/"+folderName);
	    ...
	    for (int i = 0; i < listOfFiles.length; i++) {
	      if (listOfFiles[i].isFile()) {
	        ...
	        if(candidate.substring(candidate.length()-3).equalsIgnoreCase("png") || candidate.substring(candidate.length()-3).equalsIgnoreCase("jpg")){
	        	int textInd = OpenGL.createTextureFromImage(app.loadImage(folderName+"/"+candidate));
